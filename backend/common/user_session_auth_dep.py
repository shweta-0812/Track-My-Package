...
async def authenticate_session(request: Request) -> int:
    return "shweta@parceltrack.com"
    session_key = request.cookies.get(app_settings.SESSION_COOKIE_NAME)
    if not session_key:
        raise HTTPException(
            status_code=HTTP_401_UNAUTHORIZED,
            detail=HttpRequestErrorMessages.NOT_AUTHENTICATED,
        )
    user_email = await auth_session_service.get_user_email_from_request(request=request)
    if not user_email:
        raise HTTPException(
            status_code=HTTP_401_UNAUTHORIZED,
            detail=HttpRequestErrorMessages.NOT_AUTHENTICATED,
...
async def get_current_user(user_email: str = Depends(authenticate_session)) -> Any:
    ...
    if not user:
        raise HTTPException(
            ...
            detail=UserManagerErrorMessages.USER_NOT_FOUND,
        )
    return user
...
async def get_current_active_user(
    ...
    if not current_user["status"] == PTBaseModelStatusEnum.ACTIVE.value:
        raise HTTPException(
            status_code=HTTP_401_UNAUTHORIZED,
            detail=UserManagerErrorMessages.INACTIVE_USER,
        )
